...
/*
 * UDP是不可靠的传输协议，为了减少UDP包丢失的风险，
 * 我们最好能控制UDP包在IP层的传输过程中不要被切割，
 * 在下层数据链路层MTU是1500字节的情况下要想IP层不分包，
 * 那么UDP数据包的最大大小应该是1500字节- IP头(20字节) - UDP头(8字节) = 1472字节。

 * 这也就是说IP数据报大于1500字节，大于MTU，这个时候发送方IP层就需要分片。
 * 把数据报分成若干片，使每一片都小于MTU，而接收方IP层则需要进行数据报的重组。
 * 但是由于UDP的特性，当某一片数据传送中丢失时，接收方无法重组数据报，将导致丢弃整个UDP数据报，
 * 所以通常建议UDP的数据包不要超过MTU的大小。
 *
*/
